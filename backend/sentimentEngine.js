...
// Political relevance check removed as per user request
...
/**
 * Helper to check if text contains a word with proper boundaries (for short words)
 */
function containsKeyword(text, kw) {
  ...
  // For short acronyms (<= 3 chars), use word boundaries to avoid matching sub-strings
  // Note: we check for Hindi characters separately as \b doesn't support them well
  if (lowerKw.length <= 3 && /^[a-z]+$/i.test(lowerKw)) {
 ...
 */
function analyseText(text, searchTerms = []) {
  ...
  const normalisedVal = Math.max(-1, Math.min(1, score / 3));

  // Boost user search terms
  const hasSearchTerm = searchTerms.length > 0 && searchTerms.some(term => containsKeyword(normalized, term));


  // Topic extraction
  let topics = TOPIC_MAP.filter(({ kw }) => kw.some((k) => containsKeyword(normalized, k))).map((t) => t.topic);
  
  // Add "Uttar Pradesh" topic if either the acronym UP or the full name is found
  if (containsKeyword(normalized, "UP") || containsKeyword(normalized, "Uttar Pradesh") || normalized.includes("उत्तर प्रदेश")) {
    if (!topics.includes("Uttar Pradesh")) topics.push("Uttar Pradesh");
  }

  if (hasSearchTerm) {
    const matchedTerm = searchTerms.find(term => containsKeyword(normalized, term));
    if (matchedTerm) {
      // Logic for formatting acronyms in topic list
      const formattedTerm = matchedTerm.toUpperCase() === "UP" ? "Uttar Pradesh" : matchedTerm.toUpperCase();
      if (!topics.includes(formattedTerm)) topics.unshift(formattedTerm);
    }
  }

  return {
    sentiment,
    sentiment_score: parseFloat(normalisedVal.toFixed(3)),

    topics: topics.length ? topics.slice(0, 3) : ["General"],
  };
}
